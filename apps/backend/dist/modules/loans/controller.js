...
async function getLoans(req, res) {
    try {
      ...
      `, { count: 'exact' })
        ...
        if (search)
            query = query.ilike('LG_clients.name', `%${search}%`);
        const { data, error, count } = await query;
        if (error)
            throw error;
        const loans = (data || []).map((loan) => {
            const inst = loan.LG_installments ?? loan.installments;
            const paidCount = inst?.filter((i) => i.status === 'paid').length || 0;
            const totalInst = inst?.length || 0;
            // Find next due installment
            const pendingInstallments = inst?.filter((i) => i.status === 'pending' || i.status === 'overdue' || i.status === 'partial') || [];
            return {
                ...loan,
...
async function getLoan(req, res) {
    try {
      ...
      `)
        ...
        if (inst) {
            inst.sort((a, b) => a.installment_no - b.installment_no);
            data.installments = inst;
        }
        if (data.LG_clients)
            data.clients = data.LG_clients;
...
async function createLoan(req, res) {
    try {
        ...
        const calc = (0, calculator_1.calculateLoan)(principal_amount, interest_rate, total_installments);
        // Insert loan
        ...
        const dates = (0, calculator_1.generateInstallmentDates)(start_date, due_day, total_installments);
        ...
        const { error: instError } = await supabase_1.supabase
        ...
        const { data: completeLoan } = await supabase_1.supabase
            .from('LG_loans')
