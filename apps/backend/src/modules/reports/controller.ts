...
export async function getClientReport(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    ...
    ((client.LG_loans ?? client.loans) || []).forEach((loan: any) => {
      totalLoaned += Number(loan.total_amount);
      ((loan.LG_installments ?? loan.installments) || []).forEach((inst: any) => {
    ...
    return res.json({
      ...
      loans: ((client.LG_loans ?? client.loans) || []).map((loan: any) => ({
        ...loan,
        installments: ((loan.LG_installments ?? loan.installments) || []).sort((a: any, b: any) => a.installment_no - b.installment_no),
      })),
      summary: {
...
export async function getGeneralReport(req: AuthRequest, res: Response) {
  try {
    ...
    if (start_date) transQuery = transQuery.gte('transaction_date', start_date);
    if (end_date) transQuery = transQuery.lte('transaction_date', end_date);
    ...
    (loans || []).forEach((loan: any) => {
      totalLoaned += Number(loan.total_amount);
      ((loan.LG_installments ?? loan.installments) || []).forEach((i: any) => {
    ...
    const overdueFormatted = (overdueInstallments || []).map((i: any) => {
    ...
    return res.json({
      ...
      active_loans: (loans || []).map((loan: any) => {
        const paidSum = ((loan.LG_installments ?? loan.installments) || []).reduce((s: number, i: any) => s + Number(i.paid_amount), 0);
        return {
